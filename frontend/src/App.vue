...
<template>  
  <div>
    ...
    <nav>
      ...
      <router-link to="/upload">{{ theme === 'cruelty' ? 'CRANIAL INTRUSION' : 'Upload' }}</router-link> |
      <router-link to="/gallery">{{ theme === 'cruelty' ? 'FADED MEMORIES' : 'Gallery' }}</router-link> |
      <router-link to="/search">{{ theme === 'cruelty' ? 'PAVLOVIAN RECALL' : 'Search' }}</router-link>
    </nav>
    <main>
      <suspense>
        <router-view />
    ...
    <footer>
      <p>{{ theme === 'cruelty' ? 'CONSUMER SOFTPRODUCTS // L3 // 666' : 'Created by Lucas Koumasonas and Nikita Semenov' }}</p>
    </footer>
...
.theme-toggle:hover, .cruelty-toggle:hover {
  transform: scale(1.1);
}
