...
public class AdminUserController {
  ...
  public AdminUserController(UserRepository userRepository, AppSettingRepository appSettingRepository) {
  ...
  public ResponseEntity<Map<String, Boolean>> getAutoApprove() {
    boolean val = appSettingRepository.findById("auto_approve_users")
  ...
  public ResponseEntity<?> setAutoApprove(@RequestParam boolean enabled) {
    AppSetting setting = appSettingRepository.findById("auto_approve_users")
    ...
    return userRepository.findById(id).map(user -> {
      if ("ROLE_ADMIN".equals(user.getRole())) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
          .body(Map.of("error", "Cannot delete an administrator account."));
      }
      if (user.isApproved()) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
          .body(Map.of("error", "Cannot reject an already active user. Use the ban/delete workflow instead."));
      }
